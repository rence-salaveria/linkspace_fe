  ...
  const columns = [
    ...
    columnHelper.accessor("status", {
      ...
      cell: (row) => {
        ...
        return (
          <span className={`relative inline-block px-3 py-1 font-semibold leading-tight rounded-full ${value === 'LookUp-001' ? 'bg-gray-500 text-white' : 'bg-primary text-white'}`}>
            <span aria-hidden className={`absolute inset-0 ${value === 'LookUp-001' ? 'bg-gray-500' : 'bg-primary'} opacity-50 rounded-full`}></span>
